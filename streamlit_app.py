...
if uploaded_file:
    ...
    # Gunakan gambar random dari internet jika tidak ada yang diupload
    image = Image.open(requests.get("https://picsum.photos/32/32", stream=True).raw)
...
st.subheader("Probabilitas Kelas (Persentase):")
...
# # Chart
# percentages = (prediction[0] * 100).astype(float)
# chart_data = dict(zip(class_names, percentages))
